...
- # Usage
	...
	- ## Ephemeral Diffie-Hellman (DHE)
		- DHE differs from DH in the way that static Diffie-Hellman key exchanges always use the same Diffie-Hellman private keys. So, each time the same parties do a DH key exchange, they end up with the 
		  same shared secret. DHE changes its private key in every connection and thus enables [[Forward Secrecy]], which means that if the long-term private key of the server gets leaked, past communication is still secure.
- # Vulnerabilities
